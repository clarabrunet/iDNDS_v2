   ...
   #Comparar el nombre de diferencies de nucleotids entre el codo de la sequencia de referencia i el codo de la sequencia observada
    ...
    def compare_codons(seq1, seq2, mutation_1, mutation_2, mutation_3):
        ...
        for codon1, codon2 in zip(codons1, codons2):
            ...
            total_Nd = round(total_N_1 + total_N_2 + total_N_3 + Nd_gaps, 4)
            total_Sd = round(total_S_1 + total_S_2 + total_S_3, 4)
            print(f"Codon1: {codon1}, Codon2: {codon2}, Changes: {changes}")
            print(f"total_N_1: {round(total_N_1, 4)}, total_S_1: {round(total_S_1, 4)}")
            print(f"total_N_2: {round(total_N_2, 4)}, total_S_2: {round(total_S_2, 4)}")
            print(f"total_N_3: {round(total_N_3, 4)}, total_S_3: {round(total_S_3, 4)}")
            print(f"Nd_gaps: {round(Nd_gaps, 4)}")
    ...
    def calculate_reference_values(seq, proportion_side):
        ...
        return round(total_N, 4), round(N_gaps, 4), round(total_S_ref, 4), round(total_N_ref, 4)
    ...
    Nd = round(result_mutation_1[0] + result_mutation_2[0] + result_mutation_3[0] + Nd_gaps, 4)
    Sd = round(result_mutation_1[1] + result_mutation_2[1] + result_mutation_3[1], 4)
